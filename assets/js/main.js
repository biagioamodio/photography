  ...
  $.getJSON(homeJsonPath, function(homeData) {
    ...
    displaySlides.forEach(function(slide, index) {
      ...
      $slide.on('click', function() {
        ...
        if (slide.seriesId) {
          const seriesTitle = slide.seriesTitle || slide.seriesId.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
...
function alignMetadataToImage() {
  var mq = window.matchMedia('(max-width: 1435px) and (max-height: 665px) and (orientation: landscape), (orientation: landscape) and (max-width: 932px)');
...
function applyRedViewportClass() {
  ...
  // Phone in landscape: ≤932px wide covers all iPhones, excludes tablets (1024px+)
